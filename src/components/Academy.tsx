...
interface AcademyConfig extends LandingSectionText {
  imageSrc?: string;
}

export default function Academy({ config }: { config?: AcademyConfig }) {
  return (
    <section id="academy" className="py-10 sm:py-12 lg:py-16">
      <CurtainReveal
        image={
          <ScrollColorImage
            src={config?.imageSrc || "/images/political-education.jpg"}
      ...
      >
        ...
        <p className="text-xs sm:text-sm text-dsh-desc mt-2 sm:mt-3 md:mt-4 leading-relaxed max-w-lg line-clamp-4 min-h-[4.9rem] sm:min-h-[5.7rem]">
          {config?.description ||
            "Political education built as infrastructure — a digital school, a living archive, and a space for collective learning. We share frameworks, tools, and resources to turn ideas into organised action. Free by principle."}
        </p>
        <Link href="/academy" className="mt-3 sm:mt-4 md:mt-6 inline-block self-start text-xs sm:text-sm border border-white/15 rounded-[3px] px-4 sm:px-6 py-2 sm:py-2.5 text-[#F0F0F0]/40 hover:text-[#F0F0F0]/60 hover:border-white/25 transition-colors">
          {config?.cta || "Explore Academy"} <span className="text-[#32C6CC]">+</span>
        </Link>
      </CurtainReveal>
    </section>
  );
