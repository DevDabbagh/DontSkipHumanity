...
import type { ResolvedSection } from "@/lib/page-sections";

/* ── The editable half of this page ──────────────────────────────────
   Everything a person could reasonably want to reword — the headline, the
   promises, the numbers, the project cards, the quote, the three ways to
   help — arrives from the dashboard, already in the visitor's language.
   ...
   word on this page without being able to break a payment.

   Each reader falls back to the wording that was hard-coded here before, so
   the page still reads correctly on an environment where the seed has not
   been applied. */

function sectionOf(sections: ResolvedSection[], kind: string) {
  return sections.find((s) => s.kind === kind)?.content ?? {};
}
function str(c: Record<string, unknown>, k: string): string {
  const v = c[k];
  return typeof v === "string" ? v.trim() : "";
}
function rows(c: Record<string, unknown>, k: string): Record<string, unknown>[] {
  const v = c[k];
...
export default function SupportContent({ sections }: { sections: ResolvedSection[] }) {
  return (
    <Suspense fallback={<main className="relative bg-[#0D0D0D] min-h-screen" />}>
      <SupportPageContent sections={sections} />
    </Suspense>
  );
}

function SupportPageContent({ sections }: { sections: ResolvedSection[] }) {
  ...
  /* ── Copy from the dashboard, with the original wording as the floor ── */
  const heroC  = sectionOf(sections, "hero");
  const statsC = sectionOf(sections, "stats");
  const cardsC = sectionOf(sections, "cards");
  const quoteC = sectionOf(sections, "quote");
  const waysC  = sectionOf(sections, "numbered_list");

  /* An editor may deliberately clear the label, so an empty value from the
     database is respected. But an environment where the seed has never been
     applied returns no sections at all — and that must not silently delete a
     line from the page. The two cases are told apart, not merged. */
  const seeded = sections.length > 0;
  const EYEBROW = seeded
    ? str(heroC, "eyebrow")
    : "Independent · Ad-free · Reader-supported";
  const HEADLINE = str(heroC, "headline") || "Fund the work that names power.";

  /* The second half of the headline carries the gradient. The break is
     measured from the words rather than fixed at a character index, so an
     editor can rewrite the line without the colour landing mid-word. */
  const BREAK_AT = (() => {
    const target = Math.floor(HEADLINE.length * 0.45);
    const at = HEADLINE.indexOf(" ", target);
    return at === -1 ? HEADLINE.length : at + 1;
  })();
  const HEAD_1 = HEADLINE.slice(0, BREAK_AT).trim();
  const HEAD_2 = HEADLINE.slice(BREAK_AT).trim();

  const STANDFIRST =
    str(heroC, "standfirst") ||
    "No ads. No algorithms. No sponsors shaping what gets told. This work exists because people like you make it possible.";

  const FACTS = (() => {
    const fromDb = rows(heroC, "facts").map((f) => str(f, "text")).filter(Boolean);
  ...
  const STATS = (() => {
    const fromDb = rows(statsC, "items")
      .map((i) => ({ num: str(i, "num"), label: str(i, "label") }))
      .filter((i) => i.num || i.label);
    return fromDb.length
      ? fromDb
      : [
          { num: "4,200+", label: "Free courses — forever" },
          { num: "63", label: "Emerging voices funded" },
          { num: "847", label: "Supporters made this film" },
        ];
  })();

  const CARDS = (() => {
    const fromDb = rows(cardsC, "items")
      .map((i) => ({
        label: str(i, "label"),
        title: str(i, "title"),
        sub: str(i, "meta"),
      ...
      .filter((c) => c.title);
    return fromDb.length ? fromDb : IMPACT_CARDS;
  })();

  const QUOTE =
    str(quoteC, "body") ||
    "Every film, every free course, every fellow we support — these exist because someone decided this work matters.";
  const QUOTE_BY = str(quoteC, "attribution") || "Don't Skip Humanity";
  const QUOTE_IMG = str(quoteC, "imageSrc") || "/images/note.jpg";

  /* The icons stay in code — they are drawings, not words — so a row keeps
     whichever icon sits at its position. */
  const WAYS = (() => {
    const fromDb = rows(waysC, "items")
      .map((i, n) => ({
        title: str(i, "title"),
        desc: str(i, "desc"),
        linkLabel: str(i, "linkLabel") || "Get in touch",
        icon: OTHER_WAYS[n % OTHER_WAYS.length].icon,
      }))
      .filter((w) => w.title);
    return fromDb.length ? fromDb : OTHER_WAYS.map((w) => ({ ...w, linkLabel: "Get in touch" }));
  })();

  const WAYS_LABEL = str(waysC, "label") || "Other ways to contribute";
  const STATS_LABEL = str(statsC, "label") || "The work you make possible";
  const STATS_INTRO = str(statsC, "intro") || "These stories exist because people decided they should.";
    ...
    <main className="relative bg-[#0D0D0D]">
      ...
      <div style={{ padding: "80px 32px" }}>
        <div ref={r2} className="reveal" style={{ maxWidth: 1200, margin: "0 auto" }}>
          <SectionLabel>{STATS_LABEL}</SectionLabel>
              ...
              >
                <div
                  style={{
                    position: "absolute",
                    inset: 0,
                    background: `radial-gradient(ellipse 70% 60% at 5% 95%, #B2349515 0%, transparent 60%)`,
                  }}
                />
                <p style={{ fontSize: "clamp(2rem,3vw,2.6rem)", fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.03em", lineHeight: 1 }}>
                  {s.num}
                </p>
                <p style={{ fontSize: 12, color: "#B23495", marginTop: 10, fontWeight: 500 }}>{s.label}</p>
              </div>
            ))}
          </div>
            ...
            {CARDS.map((card) => (
              ...
              >
                ...
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "24px" }}>
                  <p style={{ fontSize: 10, letterSpacing: "0.2em", color: "#B23495", textTransform: "uppercase", marginBottom: 8 }}>
                    {card.label}
                  </p>
      ...
      <div className="relative flex items-center justify-center" style={{ height: 420, isolation: "isolate" }}>
        ...
        <div ref={r3} className="reveal relative" style={{ maxWidth: 720, padding: "0 48px", textAlign: "center", zIndex: 1 }}>
          ...
          <p style={{ fontSize: 13, color: "#B23495", marginTop: 24, letterSpacing: "0.12em" }}>— {QUOTE_BY}</p>
      ...
      <div style={{ padding: "80px 32px 96px" }}>
        <div ref={r4} className="reveal" style={{ maxWidth: 1200, margin: "0 auto" }}>
          <SectionLabel>{WAYS_LABEL}</SectionLabel>
